...
public class C_Chat extends ClientBasePacket {
    ...
    public C_Chat(byte abyte0[], ClientThread clientthread) {
        ...
        if (chatType == 0) { // 一般聊天
            ...
            if (pc.getClanid() != 0) { // 在血盟中
                ...
                if ((clan != null) && ((rank == L1Clan.CLAN_RANK_GUARDIAN) || (rank == L1Clan.CLAN_RANK_LEAGUE_PRINCE)
                    ...
                    for (L1PcInstance listner : clanMembers) {
                        ...
                        if (!listner.getExcludingList().contains(pc.getName())
                                && ((listnerRank == L1Clan.CLAN_RANK_GUARDIAN)
                                        || (listnerRank == L1Clan.CLAN_RANK_LEAGUE_PRINCE)
                                        || (rank == L1Clan.CLAN_RANK_LEAGUE_VICEPRINCE)
                                        || (rank == L1Clan.CLAN_RANK_LEAGUE_GUARDIAN))) {
                            listner.sendPackets(s_chatpacket);
                        }
    ...
    private void chatWorld(L1PcInstance pc, String chatText, int chatType) {
        if (pc.isGm()) {
            ChatLogTable.getInstance().storeChat(pc, null, chatText, chatType);
            L1World.getInstance()
                    .broadcastPacketToAll(new S_ChatPacket(pc, chatText, Opcodes.S_OPCODE_GLOBALCHAT, chatType));
